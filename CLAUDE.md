...
The app requires two env vars, read at startup via `config/runtime.exs` using `System.fetch_env!` (OS environment, not auto-loaded from a file):

- `STRAVA_CLIENT_ID` — your Strava app's client ID
- `STRAVA_CLIENT_SECRET` — your Strava app's client secret

Copy `.env.example` to `.env`, fill in both values, then source it before starting the server:
...
Each line in `.env` must use the `export` prefix (as in `.env.example`) so that `source` exports vars to child processes — bare `KEY=value` lines won't be visible to `mix`.
